...
__global__ void element_wise_add_kernel(float* dest, float* src, int size) {
    ...
    if (i < size)
        dest[i] = ((float)dest[i] + (float)src[i]);
}
...
__global__ void mat_bias_vec_kernel(float* output, float* input, float* weight, float* bias, int n, int d, int numSerialElements) {
    int index = blockIdx.x * blockDim.y + threadIdx.y;
    if (index >= d)
        return;
...
__global__ void MultiHeadAttention_kernel(float* __restrict__ output, const float* __restrict__ sq,
    const float* __restrict__ key_cache, const float* __restrict__ value_cache, float* __restrict__ att_score,
...
void matmul_bias(float* xout, float* x, float* w, float* b, int n, int d) {
#if 1
    int serialElements = divUp(n, 32);
    dim3 block_dim(32, 4);
    int blocks = divUp(d, 4);
    ...
    cudaMemcpyAsync(xout, b, d*sizeof(float), cudaMemcpyDeviceToDevice);
    cublasSgemm(
        handle, CUBLAS_OP_T, CUBLAS_OP_T, dimsA.y, dimsB.x,
        dimsA.x, &alpha, w, dimsA.x, x,
        dimsB.x, &beta, xout, dimsC.y);
...
void MultiHeadAttention(float *output, float *q, float *key_cache, float *value_cache, float* att_score, int num_heads, int num_groups, int head_size, int kv_size, int loff, int seq_len) {
...
void transformer(int token, float* embedding, int pos, Config* p, RunState* s, TransformerWeights* w) {
    ...
    if(token == -1) {
        cudaMemcpyAsync(x, &embedding[pos * dim], dim * sizeof(float), cudaMemcpyDeviceToDevice);
    } else {
    ...
    cudaMemcpy(s->logits, s->logits_gpu, p->vocab_size * sizeof(float), cudaMemcpyDeviceToHost);
}
...
int main(int argc, char *argv[]) {
    ...
    float* emb_input = (float*)malloc(num_prompt_tokens * config.dim * sizeof(float));
    FILE *finput = fopen("speech_float.bin", "rb");
    fread(emb_input, sizeof(float), num_prompt_tokens * config.dim, finput);
    fclose(finput);

    float* embedding;
    cudaMalloc((void**)&embedding, num_prompt_tokens * config.dim * sizeof(float));
    cudaMemcpy(embedding, emb_input, sizeof(float) * num_prompt_tokens * config.dim, cudaMemcpyHostToDevice);
    ...
    while (pos < steps) {
        ...
        if(pos < num_prompt_tokens-1) {
            ...
            transformer(-1, embedding, pos, &config, &state, &weights);

            // if we are still processing the input prompt, force the next prompt token
            next = 198;
        } else {
            // forward the transformer to get logits for the next token
            transformer(token, NULL, pos, &config, &state, &weights);

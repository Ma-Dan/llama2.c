...
float* forward(Transformer* transformer, int token, float* embedding, int pos) {
    ...
    if(token == -1) {
        memcpy(x, &embedding[pos * dim], dim * sizeof(float));
    } else {
        // copy the token embedding into x
...
void run_transformer(int token, float* embedding, int pos, float* logits, Transformer* t) {
    float* logits_output = forward(t, token, embedding, pos);

...
void malloc_weights(TransformerWeights* w, Config* p, int shared_weights) {
    ...
    cudaMalloc((void**)&w->wk, p->n_layers * (p->dim * p->dim + p->dim) * sizeof(half));
    cudaMalloc((void**)&w->wv, p->n_layers * (p->dim * p->dim + p->dim) * sizeof(half));
...
int build_transformer(const char* checkpoint, void** p) {
    ...
    RunState* state = &(transformer->state);
    {
        ...
        if (fread(config, sizeof(Config), 1, file) != 1) { return 0; }
        ...
        transformer->shared_weights = config->vocab_size > 0 ? 1 : 0;
        config->vocab_size = abs(config->vocab_size);
        // read in the Transformer weights
        malloc_weights(weights, config, transformer->shared_weights);
        if (checkpoint_init_weights(weights, config, file, transformer->shared_weights)) { return 0; }
    }
...
void free_transformer(void* p) {
    ...
    free_weights(weights, transformer->shared_weights);
}
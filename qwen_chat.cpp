...
void memory_map_weights(TransformerWeights *w, Config* p, float* ptr, int shared_weights) {
    ...
    ptr += n_layers * p->dim;
    w->wq = ptr;
    ptr += n_layers * (p->dim * (p->n_heads * head_size) + (p->n_heads * head_size));
    w->wk = ptr;
    ptr += n_layers * (p->dim * (p->n_kv_heads * head_size) + (p->n_kv_heads * head_size));
    w->wv = ptr;
    ptr += n_layers * (p->dim * (p->n_kv_heads * head_size) + (p->n_kv_heads * head_size));
    w->wo = ptr;
...
float* forward(Transformer* transformer, int token, int pos) {
    ...
    for(unsigned long long l = 0; l < p->n_layers; l++) {
        ...
        matmul_bias(s->k, s->xb, w->wk + l*(dim*kv_dim + kv_dim), w->wk + l*(dim*kv_dim + kv_dim) + dim*kv_dim, dim, kv_dim);
        matmul_bias(s->v, s->xb, w->wv + l*(dim*kv_dim + kv_dim), w->wv + l*(dim*kv_dim + kv_dim) + dim*kv_dim, dim, kv_dim);

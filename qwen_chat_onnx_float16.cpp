...
typedef struct {
    ...
    int past_len;
    vector<vector<uint16_t>> past_key_values_key;
    vector<vector<uint16_t>> past_key_values_value;
...
void malloc_run_state(RunState* s, Config* p) {
    ...
    s->past_len = 0;
    s->past_key_values_key = vector<vector<uint16_t>>(p->n_layers);
    s->past_key_values_value = vector<vector<uint16_t>>(p->n_layers);
    for(int i=0; i<p->n_layers; i++) {
        s->past_key_values_key[i] = vector<uint16_t>(p->batch_size * p->n_heads * 1 * p->dim);
        s->past_key_values_value[i] = vector<uint16_t>(p->batch_size * p->n_heads * 1 * p->dim);
    }
...
void build_transformer(Transformer *t, const char* onnx_file_path) {
    ...
    session_option.SetIntraOpNumThreads(32);
...
void forward_transformer(Transformer* t, vector<int> tokens) {
    ...
    for(int i=0; i<c->n_layers; i++) {
        input_values.push_back(Ort::Value::CreateTensor(t->allocator_info, s->past_key_values_key[i].data(), c->n_heads * past_len * c->dim * 2, past_key_values_shape_.data(), past_key_values_shape_.size(), ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT16));
        input_values.push_back(Ort::Value::CreateTensor(t->allocator_info, s->past_key_values_value[i].data(), c->n_heads * past_len * c->dim * 2, past_key_values_shape_.data(), past_key_values_shape_.size(), ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT16));
    }
    ...
    for(int i=0; i<c->n_layers; i++) {
        ...
        memcpy(s->past_key_values_key[i].data(), output_tensors[1+i*2].GetTensorMutableData<uint16_t>(), c->n_heads * out_len * c->dim * sizeof(uint16_t));
        memcpy(s->past_key_values_value[i].data(), output_tensors[2+i*2].GetTensorMutableData<uint16_t>(), c->n_heads * out_len * c->dim * sizeof(uint16_t));
    }
}

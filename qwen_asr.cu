...
void transformer(int token, half* embedding, int pos, Config* p, RunState* s, TransformerWeights* w) {
    ...
    if(token == -1) {
        cudaMemcpyAsync(x, &embedding[pos * dim], dim * sizeof(half), cudaMemcpyDeviceToDevice);
    } else {
...
int main(int argc, char *argv[]) {
    ...
    half* emb_input = (half*)malloc(num_prompt_tokens * config.dim * sizeof(half));
    FILE *finput = fopen("speech.bin", "rb");
    fread(emb_input, sizeof(half), num_prompt_tokens * config.dim, finput);
    fclose(finput);

    half* embedding;
    cudaMalloc((void**)&embedding, num_prompt_tokens * config.dim * sizeof(half));
    cudaMemcpy(embedding, emb_input, sizeof(half) * num_prompt_tokens * config.dim, cudaMemcpyHostToDevice);
